<template>
  ...
  <div 
     ...
     >
      ...
      <h2 class="font-bold text-lg" >{{DataUser.id}}</h2>
      <h3 class="font-bold text-lg" >{{DataUser.name}}</h3>
      <p class="font-medium text-base" >{{DataUser.email}}</p>
      <p  class="font-medium text-base" > {{DataUser.role}} </p>
      ...
      <div class="action relative">
        ...
        <td class="relative">
          ...
          @click="$emit('clickShowPopup', DataUser.id)"> 
              Actions 
          ...
          <div :id="'action-' + DataUser.id" data-display="none" class="boxAction popup hidden absolute right-0 w-32 mt-1 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-50 focus:outline-none">
              <div class="px-1 py-1 popup">
                  ...
                  @click="$emit('clickEditUser', DataUser.id)">
                  ...
                  @click="$emit('clickOnDelete', DataUser.id)">
...
<script setup>
  ...
  defineEmits(['clickShowPopup', 'clickCloseAllPopup', 'clickOnDelete', 'clickEditUser'])
